...
==========================
Pulse is happy you're willing to contribute to our projects. We are usually
very lenient with all submitted PRs, but there are still some guidelines you
can follow to make the approval process go more smoothly.

## Use a Personal Fork and not an Organization

Pulse will routinely modify your PR, whether it's a quick rebase or to take care
...
- A Java 21 or later JDK (packages vary, use Google/DuckDuckGo/etc.).
    ...
    - Pulse requires JDK 21 to build, however, makes use of Gradle's
...
Unlike the Pulse API and its implementation, modifications to Paper and Minecraft source files
...
- `minecraft-patches` (applies to the `pulse-server//` git repo)
...
3. cd into `pulse-server` for server changes, `pulse-api` for API changes,
   `upstream/paper/paper-api` for Paper API changes, `upstream/paper/paper-server` for Paper Server changes,
   `upstream/purpur/purpur-api` for Purpur API changes. and `upstream/purpur/purpur-server` for Purpur Server changes,

`pulse-server/src/minecraft/java` and `pulse-server/src/minecraft/java/resources` are not git repositories in the traditional sense.
...
by the individual feature-patch commits. `upstream/paper/paper-api/` and `upstream/paper/paper-server/`
...
- `./gradlew rebuildPaperSingleFilePatches` - Fixups and rebuilds all paper single-file patches. This is how you'd make changes to the `build.gradle.kts` files located under `pulse-api` and `pulse-server`
...
1. Replace `pick` with `edit` for the commit/patch you want to modify (in this
   case the very first commit, `pulse File Patches`), and
   "save" the changes;
...
optimizations we have. This makes it easier to update Pulse during Minecraft updates,
...
Your commit will be converted into a patch that you can then PR into Pulse.
...
These steps assume the `origin` remote is your fork of this repository and `upstream` is the official PulseMC repository.
...
Remember that these changes will affect everyone who runs Pulse, not just you
and your server.
...
- You need to add a comment with a short and identifiable description of the patch:
  `// Pulse start - <COMMIT DESCRIPTION>`
...
- Multi-line changes start with `// Pulse start - <COMMIT DESCRIPTION>` and end
  with `// Pulse end - <COMMIT DESCRIPTION>`.
- One-line changes should have `// Pulse - <COMMIT DESCRIPTION>` at the end of the line.
...
Here's an example of how to mark changes by Pulse:

```java
entity.getWorld().dontBeStupid(); // Pulse - Move away from beStupid()
entity.getFriends().forEach(Entity::explode);
entity.updateFriends();

// Pulse start - Use plugin-set spawn
// entity.getWorld().explode(entity.getWorld().getSpawn());
Location spawnLocation = ((CraftWorld) entity.getWorld()).getSpawnLocation();
entity.getWorld().explode(new BlockPosition(spawnLocation.getX(), spawnLocation.getY(), spawnLocation.getZ()));
// Pulse end - Use plugin-set spawn
```
...
public class SomeVanillaClass {
    public final org.bukkit.Location newLocation; // Pulse - add location
}
```
...
but the visibility is too low (e.g. a private field in an entity class). Pulse can use access transformers
to change the visibility or remove the final modifier from fields, methods, and classes. Inside the `build-data/pulse.at`
...
When submitting feature patches to Pulse, we may ask you to add notes to the patch
...
to make future updates easier on ourselves, Pulse tries to use obfuscation
...
```java
double d0 = entity.getX(); final double fromX = d0; // Pulse - OBFHELPER
// ...   
this.someMethod(fromX); // Pulse
```
...
### Using the Pulse Test Plugin

The Pulse project has a `test-plugin` module for easily testing out API changes
...
of the other Pulse run tasks).

### Publishing to Maven local (use in external plugins)

To build and install the Pulse APIs and Server to your local Maven repository, do the following:
...
- Add `mavenLocal()` as a repository. Gradle checks repositories in the order they are declared,
  so if you also have the Pulse repository added, put the local repository above Pulse's.
...
- If you later need to use the Pulse-API, you might want to remove the jar
