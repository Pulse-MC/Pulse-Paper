        ...
        while (!this.stopping) {
            ...
            if (this.lastTick != 0 && this.timeoutTime > 0 && WatchdogThread.hasStarted && (!server.isRunning() || (currentTime > this.lastTick + this.earlyWarningEvery && !DISABLE_WATCHDOG))) { // Paper - add property to disable
                ...
                if (isLongTimeout) {
                    ...
                    logger.log(Level.SEVERE, "The server has stopped responding! This is (probably) not a Pulse bug."); // Paper // Pulse
                    ...
                    logger.log(Level.SEVERE, "If you are unsure or still think this is a Pulse bug, please report this to https://github.com/Pulse-MC/Pulse-Paper/issues"); // Pulse
                    logger.log(Level.SEVERE, "Be sure to include ALL relevant console errors and Minecraft crash reports");
                    logger.log(Level.SEVERE, "Pulse version: " + Bukkit.getServer().getVersion()); // Pulse
                    ...
                    logger.log(Level.SEVERE, "--- DO NOT REPORT THIS TO PULSE - THIS IS NOT A BUG OR A CRASH  - " + Bukkit.getServer().getVersion() + " ---"); // Pulse
                    logger.log(Level.SEVERE, "The server has not responded for " + (currentTime - lastTick) / 1000 + " seconds! Creating thread dump");
                }
                // Paper end - Different message for short timeout
                logger.log(Level.SEVERE, "------------------------------");
                logger.log(Level.SEVERE, "Server thread dump (Look for plugins here before reporting to Pulse!):"); // Paper // Pulse
                ...
                if (isLongTimeout) {
                    ...
                    logger.log(Level.SEVERE, "--- DO NOT REPORT THIS TO PULSE - THIS IS NOT A BUG OR A CRASH ---"); // Pulse
                }
