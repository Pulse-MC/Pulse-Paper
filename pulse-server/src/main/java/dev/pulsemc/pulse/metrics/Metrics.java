...
import dev.pulsemc.pulse.api.events.metrics.PulseMetricUpdateEvent;
...
public class Metrics {
    ...
    public static void start() {
        ...
        scheduler.scheduleAtFixedRate(() -> {
            ...
            // API Event
            try {
                if (PulseMetricUpdateEvent.getHandlerList().getRegisteredListeners().length > 0) {
                    double savings = Math.max(0, vanillaCpuEst - cpuUsage);
                    org.bukkit.Bukkit.getPluginManager().callEvent(
                            new PulseMetricUpdateEvent(ppsLogical, ppsPhysical, networkSpeedKbs, savings)
                    );
                }
            } catch (Exception e) {
                e.printStackTrace();
